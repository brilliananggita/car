...
if (isset($_GET['nik'])) {
    ...
    if ($result->num_rows > 0) {
        echo "<script>alert('Tidak dapat menghapus member. Terdapat transaksi yang terkait!'); window.location.href='admin.php';</script>";
    } else {
        // Query untuk menghapus member berdasarkan NIK
        $sql = "DELETE FROM tb_member WHERE nik = '$nik'";
        if ($koneksi->query($sql) === TRUE) {
            echo "<script>alert('Member berhasil dihapus!'); window.location.href='admin.php';</script>";
        } else {
